    ...
    public class PotentialMatchEditingControl : DataGridViewComboBoxEditingControl {
      ...
      protected override void OnDrawItem(DrawItemEventArgs e)
      {
        ...
        try
        {
          ...
          RectangleF titleRect = new RectangleF(e.Bounds.X, e.Bounds.Y, e.Bounds.Width - (providerSize.Width + 3),
            e.Bounds.Height);
          ...
          // paint year
          RectangleF yearRect = new RectangleF(Bounds.X + titleSize.Width + 5, e.Bounds.Y,
            e.Bounds.Width - (providerSize.Width + 3), e.Bounds.Height);
          ...
          // paint provider
          e.Graphics.FillRectangle(SystemBrushes.ControlLight, providerPosition + 1, e.Bounds.Y + 1,
            providerSize.Width, e.Bounds.Height - 3);
          e.Graphics.DrawRectangle(SystemPens.ControlDark, providerPosition + 1, e.Bounds.Y + 1, providerSize.Width,
            e.Bounds.Height - 3);
          e.Graphics.DrawString(match.Movie.PrimarySource.Provider.Name, e.Font, SystemBrushes.ControlDark,
            providerPosition + 1, e.Bounds.Y);
        }
        catch (Exception)
        {
          // ignored
